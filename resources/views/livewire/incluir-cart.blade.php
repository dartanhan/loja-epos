<div xmlns:wire="http://www.w3.org/1999/xhtml">
    ...
    @if(count($cartItems) > 0)
        <div class="table-responsive flex-grow-1 div-scroll-container">
            <div class="content">
                <table class="table table-hover table-striped text-center table-responsive" style="padding: 2px" id="table-venda">
                    ...
                    <tbody>
                        ...
                        @foreach($cartItems as $item)
                            {{-- adiciona highlighted na table se for troca--}}
                            <tr id="{{$item->id}}" data-status="{{ $item->status }}">
                                <td class="text-center">
                                    @if($item->status == \App\Enums\StatusVenda::TROCA)
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" value="" id="flexCheckDefault_{{ $item->id }}" style="display: none">
                                        </div>
                                    @else
